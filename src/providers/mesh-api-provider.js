...
function mesh2jskos(data) {
  const type = ["http://www.w3.org/2004/02/skos/core#Concept"]
  if (data["@type"]) {
    type.push(data["@type"])
  }
  const { dateIntroduced, lastUpdated, label } = data
  const concept = {
    uri: data["@id"],
    type,
    created: dateIntroduced,
    modified: lastUpdated,
    notation: [data.identifier],
...
export default class MeshApiProvider extends BaseProvider {
  ...
  static providerType = "http://bartoc.org/api-type/mesh"
  static supports = {
    schemes: true,
    data: true,
    concepts: true,
  ...
  constructor(config) {
    super(config)
    this.endpoint = config.endpoint || "https://id.nlm.nih.gov/mesh/query"
  }
  ...
  async getConcepts({ concepts }) {
    const uris = concepts?.map(c => c.uri).filter(c => c.startsWith(mesh.namespace)) || []
    return Promise.all(uris.map(uri => fetch(`${uri}.json`)
        .then(res => mesh2jskos(res.ok ? res.json() : null))
