...
class CocodaSDK {
  ...
  repeat({ function: func, interval = 15000, callback, callImmediately = true } = {}) {
    ...
    // Method to call the function and handle result/error
    const call = () => asyncFunc().then(handleResult).catch(handleError)
    ...
    // Method to set up the interval (either call immediately and then start interval, or just start interval)
