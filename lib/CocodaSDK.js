...
class CocodaSDK {
  ...
  registryForScheme(scheme) {
    ...
    for (let { type, ...config } of scheme.API || []) {
      let providerName = {
        "http://bartoc.org/api-type/jskos": "ConceptApi",
        "http://bartoc.org/api-type/skosmos": "SkosmosApi",
        "http://bartoc.org/api-type/loc": "LocApi",
      }[type]
      if (!providerName || !providers[providerName] || !providers[providerName]._registryConfigForBartocApiConfig) {
        continue
      }
